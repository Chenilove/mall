			...
			<div class="adv-table">
				<div id="dynamic-table_wrapper" class="dataTables_wrapper form-inline" role="grid">
					...
					<div class="span6" style="float: left">
							...
							<a href="/admin/goods-attr" class="btn btn-info">
								<i class="fa fa-mail-reply-all"></i>
								&nbsp;规格
							</a>	
							@if(!empty(old('query')))
							<a href="/admin/goods-attr?spec_id={{$spec_id}}" class="btn btn-success">
						...
						<div class="span6" style="float: right">
							<div class="dataTables_filter" id="dynamic-table_filter">
								<form action="/admin/goods-attr" method="get"  role='form' class="form-inline" id="query">
									...
									<input type="text" name="query" class="form-control" value="{{old('query')}}" placeholder="查询属性名(字母区分大小写)">
					...
					<tr role="row">
						...
						<th class="sorting" role="columnheader" tabindex="0" aria-controls="dynamic-table" rowspan="1" colspan="1" style="width: 267px;" aria-label="Browser: activate to sort column ascending">
							<font style="vertical-align: inherit;"><font style="vertical-align: inherit;">属性ID</font></font>
						</th>
						<th class="sorting_desc" role="columnheader" tabindex="0" aria-controls="dynamic-table" rowspan="1" colspan="1" style="width: 249px;" aria-sort="descending" aria-label="Platform(s): activate to sort column ascending">
							<font style="vertical-align: inherit;"><font style="vertical-align: inherit;">属性名称</font></font>
						</th>
						<th class="sorting_desc" role="columnheader" tabindex="0" aria-controls="dynamic-table" rowspan="1" colspan="1" style="width: 249px;" aria-sort="descending" aria-label="Platform(s): activate to sort column ascending">
					...
					<tr class="gradeC" onclick="clicks(this)">
						<td style="display:none">{{$row->attr_id}}</td>
						<td class=" ">
								<input type="checkbox" value="{{$row->attr_id}}" style="zoom:250%" id="checkk" onclick="clickd(this)">
						</td>
						<td class=" ">
							<font style="vertical-align: inherit;"><font style="vertical-align: inherit;">{{$row->attr_id}}</font></font>
						</td>
						<td class=" ">
							<font style="vertical-align: inherit;"><font style="vertical-align: inherit;"><h4 style="color:black"><b>{{$row->attr_name}}</b></h4></font></font>
						</td>
						<td class=" ">
							<span style="display:none">{{$row->attr_id}}</span><span style="display:none">{{$row->attr_name}}</span><button class="btn btn-warning" title="修改" data-toggle="modal" data-target="#edit_Modal" onclick="edit_speccification(this)">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">	
				<form role="form" action="/admin/goods-attr/2" method="post" onsubmit="return getValidate(this)" id="edit_form">
					<div class="form-group">
						<label for="exampleInputEmail1">属性名</label>
						<input type="text" name="attr_name" class="form-control" id="edit_input" placeholder="输入您所需要的属性" onblur=" validate(this) ">
						<p class="help-block" id="add_help">
							<strong>温馨提示:</strong>输入您所需要的属性
						</p>
						<input type="hidden" name="attr_id" value='' id="edit_id">
						<input type="hidden" name="spec_id" value="{{$spec_id}}">
					</div>
					{{CSRF_field()}}
					{{method_field('put')}}
...
function dels()
{
	...
	if (confirm('您确定要删除该属性吗')) {
		$.post('/admin/goods-attr-del',{'id':id,'attr':'1'},function(data){
			// /判断状态码 1 --成功 3 --数组为空
			if (data == 3) {
				alert('请选择至少一项');
				return false;
...
function validate(obj)
{
		...
		if (name == '' || name == undefined || name == null) {
			$(help).html('<span style="color:red"><strong>温馨提示:</strong>请输入属性名</span>');
			return false;
		}
